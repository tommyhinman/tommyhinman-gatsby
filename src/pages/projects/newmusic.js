...
export default function Newmusic() {
  ...
  const requestDataUrlTemplate =
    "https://7j2apoxwhf.execute-api.us-west-2.amazonaws.com/Prod/request?format=json&requestId=REQUEST_ID"

  const [currentRequestId, setCurrentRequestId] = useState("")
  const [currentRequestIndex, setCurrentRequestIndex] = useState(0)

  const [recentRequestsQuery, recentRequestsUrl, recentRequestsFetch] =
    useDataApi(
  ...
  useEffect(() => {
    const currentRequestUrl = requestDataUrlTemplate.replace(
      "REQUEST_ID",
      recentRequestsQuery.data[currentRequestIndex].requestId
    )
  ...
  const Artist = ({ artistData, primaryArtists }) => {
    ...
    return (
      <>{isPrimaryArtist ? <b>{artistData.name}</b> : <>{artistData.name}</>}</>
    )
  }
    ...
    return (
      <>
        {albumArtists.map((artist, index) => (
          <span>
            {index ? " & " : ""}
            <Artist artistData={artist} primaryArtists={primaryArtists} />
          </span>
        ))}
      </>
    )
  ...
  const FeaturingArtistsList = ({ primaryArtists }) => {
    const primaryArtistsStr = primaryArtists
      .map(artist => {
        return artist.name
      })
      .join(" & ")
  ...
  const AlbumControls = ({ albumData }) => {
    const openSpotifyLink = albumData.albumUri.replace(
      "https://open.spotify.com/",
      "spotify://"
    )
    const aotyLink =
      "https://www.albumoftheyear.org/search/albums/?q=" + albumData.albumName
  ...
  const Album = ({ albumData }) => {
    const primaryArtistIds = albumData.primaryArtists.map(primaryArtist => {
      return primaryArtist.id
    })
    const mainArtists = albumData.albumArtists.map(artist => {
      return artist.id
    })
    const mainArtistsNotPrimaryArtists = !areAnyMainArtistsInPrimaryArtists(
      ...
      <div className="panel-block">
        <p className="control columns is-vcentered is-mobile">
          ...
          <div className="column is-three-quarters-tablet is-three-fifths-mobile">
            <ArtistsList
              albumArtists={albumData.albumArtists}
              primaryArtists={primaryArtistIds}
            />
              ...
              {mainArtistsNotPrimaryArtists && (
                <>
                  ...
                  <FeaturingArtistsList
                    primaryArtists={albumData.primaryArtists}
                  />
                </>
              )}
            </span>
  ...
  const RequestInfo = ({ request }) => {
    ...
    if (request.requestTime != null) {
      formattedDate = moment(request.requestTime)
    ...
    return (
      <div className="is-size-6-mobile">
        {formattedDate}
        <span hidden> ID {request.requestId}</span>
      </div>
    )
  }
    ...
    <div>
      ...
      <Layout>
        ...
        <h2 className="subtitle is-4">
          Broken for now, until I can figure out auth issues. 😔
        </h2>
          ...
          <nav className="panel mb-5 mx-0">
            <p className="panel-heading">
              <div className="level is-mobile">
                <button
                  className="button"
                  onClick={() => previousRequestId()}
                  disabled={currentRequestIndex == numberOfRequests - 1}
                >
                  <span className="icon">
                    <HiArrowLeft />
                  </span>
